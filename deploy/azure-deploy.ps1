#!/usr/bin/env pwsh
# One-shot deploy of Seller-Admin-Tools (demo data) to Azure App Service for
# Containers.
#
...
#
# What it serves: SYNTHETIC sample data with NO authentication. That is fine for
# a demo (no PII), but before pointing it at real pipeline data, gate it behind
...
$ResourceGroup = if ($env:RESOURCE_GROUP) { $env:RESOURCE_GROUP } else { 'rg-seller-admin-tools' }
$AppName       = if ($env:APP_NAME)       { $env:APP_NAME }       else { "seller-admin-tools-$rand" }
$AcrName       = if ($env:ACR_NAME)       { $env:ACR_NAME }       else { "selleradmintacr$rand" }
$PlanName      = "plan-$AppName"
$Image         = "seller-admin-tools:latest"
$Port          = 8000
...
#    harmless for FastHTML and left enabled for deployment compatibility.
